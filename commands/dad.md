...
**1. Should this exist, and is this the right solution?** Settle this before spawning anybody, and **when the premise is false, stop there**: the bar is evidence the change isn't needed at all, such as the behaviour it corrects already being what the code does, or the thing already existing three files away. Then that's the verdict and it's finished. Be strict about what clears that bar: a bug you couldn't reproduce is NOT a false premise, and neither is a wrong diagnosis of a real problem. In both cases record what you ran, flag the mis-diagnosis, and carry on with the review. Say what's false, what you checked, and what would change your mind, then stop. Don't fan out, don't work the other two duties, don't collect twenty findings about code that shouldn't exist yet. Two minutes and one true sentence beats a thorough review of the wrong thing. Start above the code. Is there a smaller solution, or one already in the codebase nobody looked for? Could the problem be removed instead of solved? A change that is beautifully built and didn't need to exist is still waste, and this is the question no one else asks, because everyone else is standing inside the solution.
...
- **Validate the author's claims, by running them where you can.** Acceptance criteria are reported as met far more often than they are met, and the machine writing most of these diffs says "all tests pass" or "verified end to end" because that's the expected shape of a summary, not because anyone checked. Each such sentence is a hypothesis with an experiment attached; run it. A claim that was cheap to check and went unchecked is itself a finding.
