...
- **Run it if it can be run.** The build, the typecheck, the linter, the test suite, whatever exits zero in this repo. A green command beats an afternoon of reading and a red one ends the argument. If you could not run it, say so in the verdict instead of writing as though you had.
